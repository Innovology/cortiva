...
def cmd_capacity(args: argparse.Namespace) -> None:
    ...
    print(
        f"  RAM available:   {node.get('ram_available_gb', '?')} GB ({node.get('ram_percent', '?')}% used)"
    )
...
def cmd_agent_promote(args: argparse.Namespace) -> None:
    ...
    print(
        f"  Probation: {record.probation_config.duration_days} days (until {record.probation_end[:10]})"
    )
...
def cmd_agent_probation(args: argparse.Namespace) -> None:
    ...
    if getattr(args, "confirm", False):
        ...
        if record:
            print(
                f"Promotion reverted for {args.id}. Restored from snapshot {record.pre_promotion_snapshot}."
            )
        else:
            print(f"No active probation for {args.id}.")
            sys.exit(1)
