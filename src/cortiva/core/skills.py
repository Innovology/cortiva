...
class SkillRegistry:
    ...
    def load_bundled(self) -> int:
        ...
        Two sources, merged:
        1. ``skills/registry.yaml`` — the MCP-registry sync output
           (regenerated by scripts/sync_mcp_registry.py).
        2. ``skills/<name>/skill.yaml`` — first-party directory skills
           (e.g. claude_code_deep_think, github_workflow). These carry
           full procedures via ``procedures_file`` and win on name
           collision since they're hand-authored.
        """
        skills_dir = Path(__file__).parent.parent / "skills"
        count = 0
        registry_path = skills_dir / "registry.yaml"
        if registry_path.exists():
            count += self.load_file(registry_path)
        count += self.load_skill_dirs(skills_dir)
        return count

    def load_skill_dirs(self, skills_dir: Path) -> int:
        """Load directory skills (``<dir>/<name>/skill.yaml``).

        ``procedures_file`` / ``skills_text_file`` entries are resolved
        relative to the skill's directory and inlined, so installation
        works exactly like a registry skill.
        """
        ...
        for skill_yaml in sorted(skills_dir.glob("*/skill.yaml")):
            try:
                ...
                logger.warning("Unparseable skill.yaml: %s", skill_yaml)
                continue
            if not isinstance(data, dict) or "name" not in data:
                continue
            for key, target in (
                ("procedures_file", "procedures"),
                ("skills_text_file", "skills_text"),
            ):
                rel = data.get(key)
                if rel and not data.get(target):
                    content_path = skill_yaml.parent / str(rel)
                    if content_path.exists():
                        data[target] = content_path.read_text(encoding="utf-8")
