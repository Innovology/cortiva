...
def test_cooldown_blocks_recent_move():
    agents = [AgentState("dev", grade=0, current_node="m2", asleep=True, last_moved_hours_ago=1.0)]
...
def test_max_moves_cap():
    agents = [AgentState(f"d{i}", grade=0, current_node="m2", asleep=True) for i in range(5)]
