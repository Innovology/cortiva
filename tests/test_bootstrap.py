...
class TestBootstrapCommand:
    ...
    def test_bootstrap_skips_existing_agents(self, tmp_path, capsys) -> None:
        ...
        assert (
            tmp_path / "agents" / "dev-cortiva" / "identity" / "identity.md"
        ).read_text() == "existing"
...
class TestTerminalCredentialInjection:
    ...
    async def test_credentials_json_injected_into_terminal_env(
        ...
        await fabric._execute_via_terminal(
            agent,
            Task(id="t1", description="Create GitHub issue"),
        )
    ...
    async def test_no_credentials_leaves_env_untouched(self, tmp_path) -> None:
        ...
        await fabric._execute_via_terminal(
            agent,
            Task(id="t1", description="Fix the bug"),
        )
    ...
    async def test_credential_provider_merged_with_file(
        ...
        try:
            ...
            await fabric._execute_via_terminal(
                agent,
                Task(id="t1", description="Open PR"),
            )
        finally:
...
class TestDeepThinkWiring:
    ...
    async def test_deep_think_suffix_runs_and_stores(self, tmp_path, monkeypatch):
        ...
        await fabric._process_reflection(
            agent,
            Task(id="t1", description="Retirement review"),
            suffix,
        )
...
class TestAgentCommandedHiring:
    ...
    async def test_ceo_hire_provisions_new_agent(self, tmp_path):
        ...
        await fab._process_reflection(
            ceo,
            Task(id="t1", description="grow the team"),
            ReflectionSuffix(
                hire={
                    "role": "Developer",
